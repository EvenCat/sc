...
//      hashtable.put("91", "国外");
    ...
    public static boolean isCheck(String isCheck) {
        String reg = "01";
        return reg.contains(isCheck)&&isCheck.length()==1;
    }

    public static boolean fullOrAdd(String fullOrAdd) {
        String reg = "FA";
        return reg.contains(fullOrAdd) && fullOrAdd.length()==1;
    }

    public static boolean isSsq(String ssq) {
        String reg = "^[1-2][0-9]{3}[0-1][1-9]$";
        Pattern pattern = Pattern.compile(reg);
        Matcher matcher = pattern.matcher(ssq);
        return matcher.matches();
    }
     ...
     * @return
     *//*
    public static boolean cellIsNull(Row row) {
        int num = 0;
        for(int i = 0 ;i < 9; i++){
    ...
    /**
     * 判断传入的开票日期和发票代码号码是否合法
     * @param params
     * @return
     */
    public static boolean iskprqOrFpdm(String params) {
        String kprqQ = params.split("==")[0];
        String kprqZ = params.split("==")[1];
        String fpdm = params.split("==")[2];
        String fphm = params.split("==")[3];
        if(("null").equals(kprqQ) && ("null").equals(kprqZ)){
            return fpdm!=null&&fphm!=null;
        }else{
            return iskprq(kprqQ+"=="+kprqZ)&&("null").equals(fpdm)&&("null").equals(fphm);
        }
    }
     ...
     * @return
     */
    public static boolean iskprq(String kprq) {
        String kprqQ = kprq.split("==")[0];
        String kprqZ = kprq.split("==")[1];
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        Date timeQ = null;
        Date timeZ = null;
        Date nowTime = null;
        try{
            timeQ = format.parse(kprqQ);
            timeZ = format.parse(kprqZ);
            nowTime = format.parse(format.format(new Date()));
        }catch (Exception e){
            return false;
        }
        if(timeQ.getTime()>timeZ.getTime()){
            return false;
        }
        if(timeZ.getTime()>nowTime.getTime()){
            return false;
        }
        return true;
    ...
    /*public static boolean isBillInvoice(List<String> list){
        try {
            General.jsonToXml(list.get(0), list.get(1));
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
